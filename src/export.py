...
<body>
  ...
  <script>
    ...
    // `layout` bundles this render's shared, per-call-invariant values (counts,
    // peakYear, slotW, plotH, yMax, labelYears) to stay under the parameter-count limit.
    function yearBarSvg(yr, i, layout) {{
      const {{ counts, peakYear, slotW, plotH, yMax, labelYears }} = layout;
    ...
    function renderYearChart(counts) {{
      ...
      const barLayout = {{ counts, peakYear, slotW, plotH, yMax, labelYears }};
      let peakX = 0, peakBarY = YEAR_MARGIN.top;
      allYears.forEach((yr, i) => {{
        const bar = yearBarSvg(yr, i, barLayout);
        svg += bar.svg;
